...
def get_cmod_kin_profs(shot, tmin, tmax, pre_shift_TS=False, force_to_zero=False, frac_err=True, num_mc=100,
    ...
    R_ne = e.rho2rho('psinorm', 'Rmid', p_ne.X[:,0], (tmin + tmax)/2)
    R_Te = e.rho2rho('psinorm', 'Rmid', p_Te.X[:,0], (tmin + tmax)/2)
    ...
    grad_ne = get_fit_gradient(ne, ne_popt, rhop_kp, 'osborne', grad_type='analytic', out='R', reg=nereg, plot=False)
    grad_Te = get_fit_gradient(Te, Te_popt, rhop_kp, 'osborne', grad_type='analytic', out='R', reg=Tereg, plot=False)
    grad_pe = get_fit_gradient(pe, pe_popt, rhop_kp, 'osborne', grad_type='analytic', out='R', reg=pereg, plot=False)
...
def get_fit_gradient(y, c, rhop, fit_type, grad_type='analytic', wrt='R', reg=None, plot=True):
    '''Function to do X
    ...
    R = e.rho2rho('psinorm', 'Rmid', rhop_kp, (tmin + tmax)/2)
    # not yet coded: psin = 
    # not yet coded: rvol = 
    ...
    def perturb_mc_err(self, raw, num_mc, maxfev, ped_width, reg, f_rhop, ne=True, verbose=True, plot=False):
        ...
        for ii in range(max_iter):
            ...
            try:
                ...
                grad_fit_dist[ii] = get_fit_gradient(fit_dist[ii], popt, self.x, 'osborne', grad_type='analytic', out='R', reg=reg, plot=False)
