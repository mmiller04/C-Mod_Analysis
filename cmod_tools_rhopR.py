...
def get_cmod_kin_profs(shot, tmin, tmax, geqdsk=None, pre_shift_TS=False, force_to_zero=False, frac_err=True, num_mc=100,
    ...
    mode = 'H' # mode type will determine what the initial guess for the pedestal width is
    ...
    try:
        ...
        # Also pull in data in Rmid for easier computation of gradient and avoid need for propagating equilibrium info after data is pulled
        p_Te_R = profiletools.Te(int(shot), include=['ETS'], abscissa='Rmid',t_min=tmin,t_max=tmax,efit_tree=e, remove_zeros=True) # in keV
        p_ne_R = profiletools.ne(int(shot), include=['ETS'], abscissa='Rmid',t_min=tmin,t_max=tmax,efit_tree=e, remove_zeros=True) # in 1e20 m^-3
    ...
    try:
        ...
        # Rmid
        p_Te_CTS_R = profiletools.Te(int(shot), include=['CTS'], efit_tree=e, abscissa='Rmid',t_min=tmin,t_max=tmax) # in keV
        p_ne_CTS_R = profiletools.ne(int(shot), include=['CTS'], efit_tree=e, abscissa='Rmid',t_min=tmin,t_max=tmax) # in 1e20 m^-3
        ...
        p_Te.add_profile(p_Te_CTS); p_Te.add_profile(p_Te_CTS_R)
        p_ne.add_profile(p_ne_CTS); p_ne.add_profile(p_ne_CTS_R)
    ...
    p_Te_R.remove_points(np.logical_and(p_Te.X[:,0]<1.03, p_Te.y<0.015))  # use same condition as above to keep the data count consistent
    ...
    if lambdaq_type == 'scaling':
        ...
        if sub_type == 'brunner':
            ...
            Te_sep_eV, lam_q_mm = fit_2D.Teu_2pt_model(shot, tmin, tmax, geqdsk, pressure_opt = 3, lambdaq_opt=2)
        ...
        sep_dict = assemble_dict_for_2pm(shot, tmin, tmax,
                                            p_ne.X[:,0], p_ne_R.X[:,0], p_ne.y, p_ne.err_y,
                                            p_Te.X[:,0], p_Te_R.X[:,0], p_Te.y, p_Te.err_y)

        import find_separatrix as fp
        Te_sep_eV, lam_q_mm = fp.find_separatrix(sep_dict, fit_type=sub_type, plot=True)
    ...
    rhop_max_fit = 1.1 # the max value
    ...
    # Need some mapping from rhop to R to get the gradient

    # This is almost certainly not the best way to do it, as it relies on data density for ETS
    # It may be okay for certain applications, but most likely, we will want the full equilbrium information to get better gradient estimates
    rhop_to_R_raw = interp1d(p_ne.X[:,0], p_ne_R.X[:,0]) 

    grad_ne = get_fit_gradient_mapping(ne, ne_popt, rhop_kp, 'osborne', rhop_to_R_raw, grad_type='analytic', out='R', reg=nereg, plot=False)
    grad_Te = get_fit_gradient_mapping(Te, Te_popt, rhop_kp, 'osborne', rhop_to_R_raw, grad_type='analytic', out='R', reg=Tereg, plot=False)
    grad_pe = get_fit_gradient_mapping(pe, pe_popt, rhop_kp, 'osborne', rhop_to_R_raw, grad_type='analytic', out='R', reg=pereg, plot=False)
    ...
    p_ne_R_pf = build_profile_prefilter(ne_R_nofilters, ne_Y_nofilters, ne_unc_Y_nofilters, 'ne')
    p_Te_R_pf = build_profile_prefilter(Te_R_nofilters, Te_Y_nofilters, Te_unc_Y_nofilters, 'Te')

    # Make a pressure profile object
    p_pe_pf = create_pe(p_ne_pf, p_Te_pf)
    p_pe_R_pf = create_pe(p_ne_R_pf, p_Te_R_pf)
...
def get_fit_gradient_geqdsk(y, c, rhop, fit_type, geqdsk, grad_type='analytic', wrt='R', reg=None, plot=True):
    '''Function to do X
...
def get_fit_gradient_mapping(y, c, rhop, fit_type, f_rhop, grad_type='analytic', wrt='R', reg=None, plot=True):
    '''Function to do X
    ...
    jacobian = np.gradient(rhop, f_rhop(rhop))
...
class kinetic_profile:
    ...
    def perturb_mc_err(self, raw, num_mc, maxfev, ped_width, reg, f_rhop, ne=True, verbose=True, plot=False):
        ...
        for ii in range(max_iter):
            ...
            try:
                ...
                # grad_fit_dist[ii] = get_fit_gradient_mapping(fit_dist[ii], popt, self.x, 'osborne', f_rhop, grad_type='analytic', out='R', reg=reg, plot=False)
                grad_fit_dist[ii] = get_fit_gradient_geqdsk(fit_dist[ii], popt, self.x, 'osborne', geqdsk, grad_type='analytic', out='R', reg=reg, plot=False)
